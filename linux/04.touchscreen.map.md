...
比如我的电脑上 `swaymsg -t get_inputs` 输出：

```plain
Input device: Goodix Capacitive TouchScreen
  Type: Touch
  Identifier: 1046:911:Goodix_Capacitive_TouchScreen
  Product ID: 911
  Vendor ID: 1046
  Libinput Send Events: enable
```

把这个触屏映射到 eDP-1 上只需要：

```shell
swaymsg input '1046:911:Goodix_Capacitive_TouchScreen' eDP-1
```
